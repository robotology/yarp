...
* major changes to iPIDControl methods. Now they aren't bound anymore to Position
  Control. They accept a new parameter PidControlTypeEnum which defines the PID id
  on which the method will operate.
  For example `setPid(const PidControlTypeEnum& pidtype, int j, const Pid &pid)`.
  Currently, pidtype can be one of the following types: `VOCAB_PIDTYPE_POSITION`,
  `VOCAB_PIDTYPE_VELOCITY`, `VOCAB_PIDTYPE_TORQUE`, `VOCAB_PIDTYPE_CURRENT`.
  See issue https://github.com/robotology/yarp/issues/499
* All pid-related methods from interfaces different from iPidControl have been deprecated.
  To provide backward-compatibility, old methods automatically call the new methods
...
* Replaced `QT_DEVICE_PIXEL_RATIO` with `QT_AUTO_SCREEN_SCALE_FACTOR` when
  supported by the installed Qt5 version. (#837, #1122, #1038)
