...
public:
    ...
    void checkMiscOperationsFixedSize() {
        report(0,"check matrix misc operations for fixed size vectors...");
        Matrix mm=eye(5,2);
        Matrix mt=mm.transposed();

        const int n = 3, m=2;
        VectorN<n> rangeMin(-100);
        VectorN<n> rangeMax(100);
        double s = Rand::scalar(rangeMin[0], rangeMax[0]);    // scalar
        double s2 = Rand::scalar(rangeMin[0], rangeMax[0]);   // scalar
        VectorN<1> a1 = Rand::vector(rangeMin.subVector(0,0), rangeMax.subVector(0,0));   // 1 dim vector
        VectorN<1> b1 = Rand::vector(rangeMin.subVector(0,0), rangeMax.subVector(0,0));   // 1 dim vector
        VectorN<1> c1 = Rand::vector(rangeMin.subVector(0,0), rangeMax.subVector(0,0));   // 1 dim vector
        VectorN<n> an = Rand::vector(rangeMin, rangeMax);         // n dim vector
        VectorN<n> bn = Rand::vector(rangeMin, rangeMax);         // n dim vector
        VectorN<n> cn = Rand::vector(rangeMin, rangeMax);         // n dim vector
        Matrix An = Rand::matrix(n,1), Bn = Rand::matrix(n,1);
        Matrix Ann = Rand::matrix(n,n), Amn = Rand::matrix(m,n), Bmn = Rand::matrix(m,n), Cmn = Rand::matrix(m,n);
        printf("(x y z are scalar; a b c are vectors; A B C are matrices)\n");
        string testName = "a(x+y) = ax+ay"; assertEqual(an*(s+s2), an*s + an*s2, testName);
        testName = "a(b+c) = ab+ac"; 
        assertEqual(an*(bn+cn), an*bn + an*cn, testName);
        testName = "a(b+x) = ab+ax"; assertEqual(an*(bn+s), an*bn + an*s, testName);
        testName = "a(x+b) = ax+ab"; assertEqual(an*(s+bn), an*s + an*bn, testName);
        testName = "A(B+C) = AB+AC"; assertEqual(Amn.transposed()*(Bmn+Cmn), Amn.transposed()*Bmn + Amn.transposed()*Cmn, testName);
    }
    ...
    void vectorOpsFixedSize()
    {
        report(0,"checking vector operators for fixed size...");
    ...
    void vectMatrix()
    {
        report(0,"checking vector matrix operators...");
        Matrix m(3,2);
        m=eye(3,2);
        Vector v1(3);
        v1=1;
    ...
    void vectMatrixFixedSize()
    {
        report(0,"checking vector matrix operators for fixed size...");
        Matrix m(3,2);
        m=eye(3,2);
        Vector3 v1;
        v1=1;
    ...
    void sumSubtractionOperatorsFixedSize() {
        report(0,"checking sum and subtraction operators for fixed size...");
        const int n = 3, m=2;
        VectorN<n> rangeMin(-100);
        VectorN<n> rangeMax(100);
        double s = Rand::scalar(rangeMin[0], rangeMax[0]);    // scalar
        Vector1 a1 = Rand::vector(rangeMin.subVector(0,0), rangeMax.subVector(0,0));   // 1 dim vector
        Vector1 b1 = Rand::vector(rangeMin.subVector(0,0), rangeMax.subVector(0,0));   // 1 dim vector
        Vector1 c1 = Rand::vector(rangeMin.subVector(0,0), rangeMax.subVector(0,0));   // 1 dim vector
        VectorN<n> an = Rand::vector(rangeMin, rangeMax);         // n dim vector
        VectorN<n> bn = Rand::vector(rangeMin, rangeMax);         // n dim vector
        VectorN<n> cn = Rand::vector(rangeMin, rangeMax);         // n dim vector
        Matrix An = Rand::matrix(n,1), Bn = Rand::matrix(n,1);
    ...
    void productOperatorFixedSize()
    {
        report(0,"checking product operator for fixed size...");
        const int n = 3, m=2;
        VectorN<n> rangeMin(-100);
        VectorN<n> rangeMax(100);
        double s = Rand::scalar(rangeMin[0], rangeMax[0]);    // scalar
    ...
    void divisionOperatorFixedSize()
    {
        report(0,"checking division operator for fixed size...");
        const int n = 3, m=2;
        VectorN<n> rangeMin(-100);
        VectorN<n> rangeMax(100);
        double s = Rand::scalar(rangeMin[0], rangeMax[0]);    // scalar
    ...
    void crossProductFixedSize()
    {
        report(0,"checking cross product for fixed size...");
        Vector3 rangeMin(-100);
        Vector3 rangeMax(100);
        Vector3 an = Rand::vector(rangeMin, rangeMax);         // 3 dim vector
        Vector3 bn = Rand::vector(rangeMin, rangeMax);         // 3 dim vector
        Matrix A(3,3);
        double ann = norm(an);
        double bnn = norm(bn);

        string testName = "a x a = 0";
        assertEqual(cross(an,an), zeros(3), testName);
        testName = "norm(a x b) = norm(a)*norm(b)*sin(theta)";
        assertEqual(norm(cross(an,bn)), ann*bnn*sqrt(1.0-pow(dot(an,bn)/(ann*bnn),2)), testName.c_str());
        testName = "cross product = cross product matrix";
        assertEqual(cross(an, bn), crossProductMatrix(an)*bn, testName);
    ...
    void subVectorFixedSize()
    {
        report(0,"subvector for fixed size...");
        const int n = 10, m=5, s=(int)Rand::scalar(0,5), k=(int)Rand::scalar(0,5);
        VectorN<n> rangeMin(-100);
        VectorN<n> rangeMax(100);
        Vector a = Rand::vector(rangeMin, rangeMax);            // n dim vector
        VectorN<n> b = Rand::vector(rangeMin, rangeMax);        // n dim vector
        VectorN<m> bSub = b.subVector<m>(s);
        VectorN<m> aSub = a.subVector<m>(s);

        string testName = "a[s:s+5][k] = a[s+k]";
        assertEqual(aSub[k], a[s+k], testName);
        testName = "b[s:s+5][k] = b[s+k]";
        assertEqual(bSub[k], b[s+k], testName);
    }
    ...
    virtual void runTests() 
    {
        ...
        checkMiscOperationsFixedSize();
        vectorOpsFixedSize();
        vectMatrixFixedSize();
        sumSubtractionOperatorsFixedSize();
        productOperatorFixedSize();
        divisionOperatorFixedSize();
        crossProductFixedSize();
        subVectorFixedSize();
    }
};
