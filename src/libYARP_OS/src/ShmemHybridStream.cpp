...
int yarp::ShmemHybridStream::accept()
{
	...
	m_pSendMap=new ACE_Mem_Map(send_conn_data.filename, //const ACE_TCHAR *filename,
	...
	m_pSendBuffer=(char*)m_pSendMap->addr();
#else
	...
	{
		YARP_ERROR(Logger::get(),"ShmemHybridStream server returned %d",ret);
		Close();
		return -1;
	}
	...
	m_pRecvMap=new ACE_Mem_Map(recv_conn_data.filename, //const ACE_TCHAR *filename,
	...
	m_pRecvBuffer=(char*)m_pRecvMap->addr();
#else
	shmemkey=2*m_Port+1;
...
int yarp::ShmemHybridStream::connect()
{
	...
	{
		YARP_ERROR(Logger::get(),"ShmemHybridStream: client returned %d",ret);
		Close();
		return -1;
	}
	...
	// ora deve mandare i dati al server
	...
	m_pRecvMap=new ACE_Mem_Map(send_conn_data.filename, //const ACE_TCHAR *filename,
		m_SendBuffSize, //int len = -1,
		O_RDWR | O_CREAT, //int flags = O_RDWR | O_CREAT,
		ACE_DEFAULT_FILE_PERMS, //int mode = ACE_DEFAULT_FILE_PERMS,
		PROT_RDWR, //int prot = PROT_RDWR,
	...
	m_pSendBuffer=(char*)m_pRecvMap->addr();
