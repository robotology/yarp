...
TEST_CASE("dev::frameTransformGet_nws_yarp", "[yarp::dev]")
{
#if defined(DISABLE_FAILING_TESTS)
    YARP_SKIP_TEST("Skipping failing tests")
#endif

        YARP_REQUIRE_PLUGIN("frameTransformGet_nws_yarp", "device");

    yarp::os::Network::setLocalMode(true);
    const bool verboseDebug = true;

    SECTION("test the frameTransformGet_nws_yarp alone")
    {
        yarp::dev::PolyDriver pd;
        yarp::os::Property p;

        p.put("device", "frameTransformGet_nws_yarp");
        REQUIRE(pd.open(p));
    ...
    SECTION("test the frameTransformGet_nws_yarp")
    {
        ...
        p_nws.put("device", "frameTransformGet_nws_yarp");
