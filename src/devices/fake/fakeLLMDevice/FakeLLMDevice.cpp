...
ReturnValue FakeLLMDevice::setPrompt(const std::string &prompt)
{
    ...
    m_conversation.push_back(LLM_Message("system", prompt,{},{}));
...
ReturnValue FakeLLMDevice::ask(const std::string &question, LLM_Message &oAnswer)
{
    ...
    if(question == "function")
    {
        ...
        answer = LLM_Message("function",function_name,function_params,function_args);
    }
    else
    {
        std::string answer_content = "Fatti non foste per viver come bruti ma per seguir virtute e canoscenza";
        answer = LLM_Message("assistant", answer_content,{},{});
    }
    m_conversation.push_back(LLM_Message("user", question,{},{}));
    m_conversation.push_back(answer);
    oAnswer = answer;
    return ReturnValue_ok;
}

ReturnValue FakeLLMDevice::getConversation(std::vector<LLM_Message>& oConversation)
{
