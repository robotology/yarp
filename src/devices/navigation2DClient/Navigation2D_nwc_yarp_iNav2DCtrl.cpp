...
bool Navigation2D_nwc_yarp::getNavigationStatus(NavigationStatusEnum& status)
{
    ...
    if (ret)
    {
        ...
        {
            yCError(NAVIGATION2D_NWC) << "getNavigationStatus() received error from navigation server";
            return false;
        }
        else
        {
    ...
    {
        yCError(NAVIGATION2D_NWC) << "getNavigationStatus() error on writing on rpc port";
        return false;
    }
    return true;
}
...
bool  Navigation2D_nwc_yarp::recomputeCurrentNavigationPath()
{
    ...
    if (ret)
    {
        if (resp.get(0).asVocab32() != VOCAB_OK)
        {
            yCError(NAVIGATION2D_NWC) << "recomputeCurrentNavigationPath() received error from navigation server";
            return false;
        }
    }
    else
    {
        yCError(NAVIGATION2D_NWC) << "recomputeCurrentNavigationPath() error on writing on rpc port";
        return false;
    }
    return true;
}
...
bool Navigation2D_nwc_yarp::suspendNavigation(const double time_s)
{
    ...
    if (ret)
    {
        if (resp.get(0).asVocab32() != VOCAB_OK)
        {
            yCError(NAVIGATION2D_NWC) << "suspendNavigation() received error from navigation server";
            return false;
        }
    }
    else
    {
        yCError(NAVIGATION2D_NWC) << "suspendNavigation() error on writing on rpc port";
        return false;
    }
    return true;
}
...
bool Navigation2D_nwc_yarp::stopNavigation()
{
    ...
    if (ret)
    {
        if (resp.get(0).asVocab32() != VOCAB_OK)
        {
            yCError(NAVIGATION2D_NWC) << "stopNavigation() received error from navigation server";
            return false;
        }
    }
    else
    {
        yCError(NAVIGATION2D_NWC) << "stopNavigation() error on writing on rpc port";
        return false;
    }
    return true;
}
...
bool Navigation2D_nwc_yarp::resumeNavigation()
{
    ...
    if (ret)
    {
        if (resp.get(0).asVocab32() != VOCAB_OK)
        {
            yCError(NAVIGATION2D_NWC) << "resumeNavigation() received error from navigation server";
            return false;
        }
    }
    else
    {
        yCError(NAVIGATION2D_NWC) << "resumeNavigation() error on writing on rpc port";
        return false;
    }
    return true;
}
...
bool   Navigation2D_nwc_yarp::getAllNavigationWaypoints(yarp::dev::Nav2D::TrajectoryTypeEnum trajectory_type, yarp::dev::Nav2D::Map2DPath& waypoints)
{
    ...
    if (ret)
    {
        if (resp.get(0).asVocab32() != VOCAB_OK)
        {
            yCError(NAVIGATION2D_NWC) << "getNavigationWaypoints() received error from navigation server";
            return false;
        }
        else if (resp.get(1).isList() && resp.get(1).asList()->size()>0)
        {
            ...
            if (waypoints_bottle == 0) { yCError(NAVIGATION2D_NWC) << "getNavigationWaypoints parsing error"; return false; }
            for (size_t i = 0; i < waypoints_bottle->size(); i++)
            {
                Bottle* the_waypoint = waypoints_bottle->get(i).asList();
                if (the_waypoint == 0) { yCError(NAVIGATION2D_NWC) << "getNavigationWaypoints parsing error"; return false; }
                Map2DLocation loc;
    ...
    {
        yCError(NAVIGATION2D_NWC) << "getCurrentPosition() error on writing on rpc port";
        return false;
    }
    return true;
}
...
bool   Navigation2D_nwc_yarp::getCurrentNavigationWaypoint(Map2DLocation& curr_waypoint)
{
    ...
    if (ret)
    {
        if (resp.get(0).asVocab32() != VOCAB_OK)
        {
            yCError(NAVIGATION2D_NWC) << "getCurrentNavigationWaypoint() received error from navigation server";
            return false;
        }
        else if (resp.size() == 5)
        {
...
bool Navigation2D_nwc_yarp::getCurrentNavigationMap(yarp::dev::Nav2D::NavigationMapTypeEnum map_type,MapGrid2D& map)
{
    ...
    if (ret)
    {
        if (resp.get(0).asVocab32() != VOCAB_OK)
        {
            yCError(NAVIGATION2D_NWC) << "getCurrentNavigationMap() received error from server";
            return false;
        }
        else
        {
            ...
            {
                yCError(NAVIGATION2D_NWC) << "getCurrentNavigationMap() failed copyPortable()";
                return false;
            }
        }
    }
    ...
    {
        yCError(NAVIGATION2D_NWC) << "getCurrentNavigationMap() error on writing on rpc port";
        return false;
    }
    return true;
}
