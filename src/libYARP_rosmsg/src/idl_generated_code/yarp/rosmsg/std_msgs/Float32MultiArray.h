...
public:
    ...
    std::vector<yarp::conf::float32_t> data;
    ...
    bool readBare(yarp::os::ConnectionReader& connection) override
    {
        ...
        data.resize(len);
        if (len > 0 && !connection.expectBlock((char*)&data[0], sizeof(yarp::conf::float32_t)*len)) {
            return false;
        }
    ...
    bool readBottle(yarp::os::ConnectionReader& connection) override
    {
        ...
        data.resize(len);
        for (int i=0; i<len; i++) {
            data[i] = (yarp::conf::float32_t)connection.expectFloat32();
        }
    ...
    bool writeBare(yarp::os::ConnectionWriter& connection) const override
    {
        ...
        if (data.size()>0) {
            connection.appendExternalBlock((char*)&data[0], sizeof(yarp::conf::float32_t)*data.size());
        }
    ...
    bool writeBottle(yarp::os::ConnectionWriter& connection) const override
    {
        ...
        for (size_t i=0; i<data.size(); i++) {
            connection.appendFloat32(data[i]);
        }
