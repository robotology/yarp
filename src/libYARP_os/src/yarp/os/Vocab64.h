 ...
 *
 * Use as, for example, yarp::os::createVocab64('s','e','t')
 *
 ...
 */
constexpr yarp::conf::vocab64_t createVocab64(char a,     char b = 0, char c = 0, char d = 0,
                                              char e = 0, char f = 0, char g = 0, char h = 0)
{
    return (static_cast<yarp::conf::vocab64_t>(a))       +
           (static_cast<yarp::conf::vocab64_t>(b) << 8)  +
           (static_cast<yarp::conf::vocab64_t>(c) << 16) +
           (static_cast<yarp::conf::vocab64_t>(d) << 24) +
           (static_cast<yarp::conf::vocab64_t>(e) << 32) +
           (static_cast<yarp::conf::vocab64_t>(f) << 40) +
           (static_cast<yarp::conf::vocab64_t>(g) << 48) +
           (static_cast<yarp::conf::vocab64_t>(h) << 56);
}

/**
 * Short readable codes.  They are integers, for efficient switching,
 * but have a readable (and writable) string representation.
 * Codes are limited to 8 characters long (to fit in a 64 bit network integer, NetInt64).
