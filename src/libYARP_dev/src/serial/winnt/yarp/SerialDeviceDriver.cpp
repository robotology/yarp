...
#include "../dd_orig/include/SerialHandler.h"
#include "../dd_orig/include/SerialFeedbackData.h"
...
{
    if(RES(system_resources).initialize(config.CommChannel.c_str(), config.SerialParams) < 0)
        return false;
    return true;
}
...
bool SerialDeviceDriver::open(yarp::os::Searchable& config) {
    ...
    config2.CommChannel = config.check("comport",Value("COM3"),"name of the serial channel").asString();
...
bool SerialDeviceDriver::send(const Bottle& msg)
{
    ...
    const char *message = msg.toString().c_str();
